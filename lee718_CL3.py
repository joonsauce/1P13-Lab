...
def class_average(final_marks):
    ...
    for i in final_marks:
        avg += extract_grade(i)
    return avg/len(final_marks)
    
#********************************************
# Write your class_std_dev function below: (3 marks)
#********************************************
def class_std_dev(final_marks):
    avg = class_average(final_marks)
    std = float()
    for i in final_marks:
        std += (extract_grade(i) - avg)**2
    return (std/len(final_marks))**0.5
...
# %% [markdown]
# ```
# 1. If there is no input, len(final_marks) will be zero and cause a ZeroDivisionError. However, other than that, there should be no input that causes the program to fail given that the input is formatted correctly all the time.
# 2. Change the .split() function from stripping away a hyphen to split away a colon --> string.split('-') vs. string.split(':'). After that, the .strip('%') function will need to be removed, as there is no longer a percent sign to worry about.
# 3. No, since a separate loop will be required for calculating the average and the standard deviation. This is because the standard deviation calculation requires the average to have been calculated before it can be started.
# ```
